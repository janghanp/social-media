...
const ControlMenu = ({ setToggleControlMenu, post }: Props) => {
  const [toggleConfirm, setToggleConfirm] = useState<boolean>(false);

  const deleteHandler = async () => {
    //Send a http request to delete the post.
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
      {toggleConfirm ? (
        ...
        <ul className="menu w-4/5 max-w-md rounded-md border border-primary bg-base-100 shadow-md">
          <li className="p-3 text-center transition duration-200 hover:cursor-pointer hover:bg-gray-200">
            edit
          </li>
          <li
            onClick={() => setToggleConfirm(true)}
            className="p-3 text-center font-semibold text-warning transition duration-200 hover:cursor-pointer hover:bg-gray-200"
          >
            delete
          </li>
          <li
